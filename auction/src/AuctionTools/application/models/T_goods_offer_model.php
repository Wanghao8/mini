...
/**
 * Class T_goods_offer_model
 * 拍品出价模型 model @lin
 */

class T_goods_offer_model extends MY_model{
    var $table = 't_goods_offer';
    ...
    //返回带头像昵称的列表
    public function do_offer_list($goods_id){
        $table = $this->table;
        $sql = "SELECT a.*,b.nickName,b.avatarUrl FROM ".$table." AS `a` INNER JOIN `t_manage_acc` AS `b` ON `a`.`acc_id` = `b`.`id` WHERE goods_id = ".$goods_id." ORDER BY a.id DESC";
        $row=$this->getRows($sql);
        return $row ;
    }
