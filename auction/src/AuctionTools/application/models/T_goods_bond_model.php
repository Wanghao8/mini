...
/**
 * Class T_goods_bond_model
 * 拍品保证金模型 model @lin
 */

class T_goods_bond_model extends MY_model{
    var $table = 't_goods_bond';
    ...
    //保证金列表
    public function do_bond_list($where,$page,$limit,$field='*'){
        $table=$this->table;
        $limits=($page==0)?'':('limit '.($page-1)*$limit.','.$limit.'');
        $where=(empty($where))?'':('where ' .$where);
        $sql='select '.$field.' from '.$table.' as a left join t_goods as b on a.goods_id=b.id '.$where.' '.$limits;
        $row=$this->getRows($sql);
        return $row ;
    }
