...
class Gateway
{
    ...
    protected static function getBufferFromGateway($gateway_buffer_array)
    {
        ...
        foreach ($gateway_buffer_array as $address => $gateway_buffer) {
            $client = stream_socket_client("tcp://47.104.195.149:2700", $errno, $errmsg, static::$connectTimeout);
    ...
    protected static function sendBufferToGateway($address, $gateway_buffer)
    {
        ...
        $client         = stream_socket_client("tcp://47.104.195.149:2700", $errno, $errmsg, static::$connectTimeout, $flag);
...
class Context
{
    ...
    public static function clientIdToAddress($client_id)
    {
        ...
        $res = unpack('Nlocal_ip/nlocal_port/Nconnection_id' ,pack('H*', $client_id));
        if ($res['local_ip'] == 0){
            $res['local_ip'] = '795394965';
        }
