...
abstract class Wechat implements GatewayInterface
{
    ...
    public function refund($config_biz = [])
    {
        ...
        return $this->getResultRefund($this->gateway_refund, true);
    }
     ...
     * @return array
     */
    protected function getResultRefund($path, $cert = false)
    {
        $this->config['sign'] = $this->getSign($this->config);

        if ($cert) {
            $data = $this->fromXml($this->post_refund(
                $this->endpoint.$path,
                $this->toXml($this->config),
                [
                    'cert'    => $this->user_config->get('cert_client', ''),
            ...
            $data = $this->fromXml($this->post_refund($this->endpoint.$path, $this->toXml($this->config)));
        }
